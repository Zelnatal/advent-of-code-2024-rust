...
fn evaluate(expect: i64, cur: i64, mut next: std::slice::Iter<'_, i64>) -> bool {
    if let Some(&next_num) = next.next() {
        evaluate(expect, cur + next_num, next.clone()) || evaluate(expect, cur * next_num, next)
    } else {
        expect == cur
    }
}
...
mod tests {
    ...
    #[test]
    fn exemple() {
        let input = read_to_string("./input/day7/1_exemple.txt").unwrap();
        assert_eq!(solver(&input), 3749)
    }
}
